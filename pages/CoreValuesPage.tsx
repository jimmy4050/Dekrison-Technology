...
const ValueCard: React.FC<{ icon: React.ReactNode; title: string; description: string }> = ({ icon, title, description }) => (
  <div className="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 text-center h-full">
    <div className="inline-block p-5 bg-[#FFD447]/20 dark:bg-[#FFD447]/10 rounded-full mb-6">
      {icon}
    </div>
    <h3 className="text-2xl font-bold text-[#0A2342] dark:text-white mb-3">{title}</h3>
...
const CoreValuesPage: React.FC = () => {
  return (
    <div className="py-16 md:py-20 bg-[#F8F9FA] dark:bg-gray-900">
      <div className="container mx-auto px-6">
        <AnimatedSection className="text-center">
          <h1 className="text-4xl md:text-5xl font-bold mb-4 text-[#0A2342] dark:text-white">Our Core Values</h1>
        ...
        <AnimatedSection className="mt-24 text-center">
          <p className="text-2xl md:text-3xl font-semibold text-[#0A2342] dark:text-gray-100 italic">"Light up the wild, shine with quality."</p>
        </AnimatedSection>
      </div>
    </div>
  );
